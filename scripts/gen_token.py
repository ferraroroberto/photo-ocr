...
Why this exists
---------------
The webapp's Cloudflare tunnel puts the photo OCR app on a public URL.
Even behind Cloudflare Access, the bearer token adds a second factor
on the API itself — a caller past the Access policy still needs the
token to upload photos and run OCR on your hardware.
...
it from the visible URL. All later visits authenticate from
localStorage — nothing to type.

Rotation = re-run with --force, then re-open the new tokenised URL
once on each device that should keep working. Old devices stop
working immediately.
...
def main() -> int:
    ...
    print("• Loopback callers keep working without the token.")
